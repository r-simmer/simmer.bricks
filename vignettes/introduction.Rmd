...
t <- trajectory() %>%
  ...
  delayed_release(env, "res1", 2) %>% 
  log_("res1 released") %>%
  # inoperative for 5 units of time
  delayed_release(env, "res2", 5, preemptive=TRUE) %>%
  log_("res1 released")
